...
class My_Links {
	...
	function init_links_content_type() {
		...
		add_action( 'pre_get_posts',  array( $this, 'order_link_posts' ) );
	...
	public function add_content_type() {
		$labels = array(
			'name'			   => _x( 'Press Links', 'post type general name' ),
			'singular_name'	  => _x( 'Press Links', 'post type singular name' ),
			'add_new'			=> __( 'Add New' ),
			'add_new_item'	   => __( 'Add New Link' ),
			'edit_item'		  => __( 'Edit Link' ),
			'new_item'		   => __( 'New Link' ),
			'all_items'		  => __( 'All Press Links' ),
		...
		$args = array(
			...
			'has_archive'		 => false,
			//'rewrite'			  => array( 'slug' => 'press-room/%my_links_category%', 'with_front' => false ),
			'rewrite'			 => array( 'slug' => 'press-link', 'with_front' => false ),
	...
	public function add_taxonomies() {
		...
		$args = array(
			'labels'			  => $labels,
			'hierarchical'		=> true,
			'rewrite'			 => array( 'slug' => 'about-us/press-room', 'with_front' => false )
		);
	...
	public function my_links_date_meta_box_content( $post ) {
		...
		$html = '<p>';
		$html .= '<input class="datepicker" type="text" name="link-date" id="link-date" style="width:100%" value="'.( !empty($value) ? date('m/d/Y', strtotime($value)) : '' ).'" />';
	...
	public function my_links_target_meta_box_content( $post ) {
		...
		$html = '<p>';
		$html .= '<p><label for="link_target_blank" class="selectit"><input id="link_target_blank" type="radio" name="link-target" value="_blank"
		'.( (!empty($value) && $value == '_blank') ? 'checked="checked"' : '' ).' /><code>_blank</code> &mdash; new window or tab</label></p><p><label for="link_target_top" class="selectit"><input id="link_target_top" type="radio" name="link-target" value="_top"
		'.( (!empty($value) && $value == '_top') ? 'checked="checked"' : '' ).' />
<code>_top</code> &mdash; current window or tab, with no frames</label></p><p><label for="link_target_none" class="selectit"><input id="link_target_none" type="radio" name="link-target" value="_none" '.( (!empty($value) && $value == '_none') ? 'checked="checked"' : '' ).' /><code>_none</code> &mdash; same window or tab</label></p>';
	...
	public function save_custom_meta_box( $post_id ) {
		...
		if ( isset($_POST['link-date']) && !empty($_POST['link-date']) ) {
			update_post_meta( $post_id, '_link-date', date('Y-m-d', strtotime(sanitize_text_field( $_POST[ 'link-date' ] ))) );
		}
	...
	function press_links_shortcode( $atts ) {
		...
		if ($loop) {
			...
			while ($loop->have_posts()) {
				...
				$output .= '<li>';
				if ($date) {
					$output .= ''.sprintf(__('<time class="updated" datetime="%1$s" pubdate>%2$s</time>', 'scaffolding'), $date, date('m/d/y', strtotime($date))).' ';
				}
			...
			if ($category !== '' && $category !== 'all') {
				...
				$output .= '<li><a class="read-more" href="'.$link.'" title="'.$term->name.' Archive">Read More &raquo;</a></li>';
			}
	...
	public function order_link_posts( $query ) {
		if($query->is_main_query() && is_tax('my_links_category')) {
			// set order for archive pages
			$query->set( 'orderby', 'meta_value' );
			$query->set( 'meta_type', 'DATE' );
			$query->set( 'meta_key', '_link-date' );
